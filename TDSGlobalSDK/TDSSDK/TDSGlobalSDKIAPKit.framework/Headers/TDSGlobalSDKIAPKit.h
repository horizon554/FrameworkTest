//
//  TDSSDKIAPKit.h
//  TDSSDKIAPKit
//
//  Created by JiangJiahao on 2020/10/26.
//

#import <Foundation/Foundation.h>

//! Project version number for TDSSDKIAPKit.
FOUNDATION_EXPORT double TDSSDKIAPKitVersionNumber;

//! Project version string for TDSSDKIAPKit.
FOUNDATION_EXPORT const unsigned char TDSSDKIAPKitVersionString[];

// In this header, you should import all the public headers of your framework using statements like #import <TDSSDKIAPKit/PublicHeader.h>

#import <TDSGlobalSDKIAPKit/TDSGlobalInAppPurchaseManager.h>

